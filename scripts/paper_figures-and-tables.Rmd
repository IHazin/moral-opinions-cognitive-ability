...
models_data <- gss_by_issue %>%
  mutate(model = map(data, ~model_function(.))) %>%
  select(-data)

# Unnest 'models_data' to get coefficients:
coefficients <- models_data %>%
  unnest() %>% 
...
cor.test(coefficients$hvfl_advantage, coefficients$time)
cor.test(coefficients$hvfl_advantage, coefficients$polviews_cont_sc)
cor.test(coefficients$hvfl_advantage, coefficients$wordsum_sc)
...
panelC <- predictions_C %>% 
  ...
  ggplot(aes(x = wordsum_sc, y = adjusted_predictions, group = issue, color = hvfl_advantage)) +
  geom_line(size = 0.75) +
  scale_colour_gradient(low = "red", high = "blue") +
  ylim(0.1, 0.9) +
...
# For each issue, run 'model_function':
models_data <- gss_by_issue_polviews %>%
  mutate(model = map(data, ~model_function(.))) %>%
  select(-data)
  
# Unnest 'models_data' to get the effect of cognitive ability:
coefficients <- models_data %>%
  unnest() %>% 
